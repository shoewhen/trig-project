...
class sin_equals_one_half(Scene):
    def construct(self):
        ...
        y_line_left = DashedLine(axes_left.c2p(-np.sqrt(3)/2, 1/2), axes_left.c2p(0, 1/2)).set_color(GREEN)
        y_line = DashedLine(axes_right.c2p(np.sqrt(3)/2, 1/2), axes_right.c2p(0, 1/2)).set_color(GREEN)

        equation1 = MathTex(r"\sin\left(\theta\right)=\frac{1}{2}", font_size=30).move_to(np.array([-5, 3, 0]))
        one_half_left = MathTex(r"\frac{1}{2}", font_size=30, color=GREEN).move_to(axes_left.c2p(0.2, 1/2))
        one_half = MathTex(r"\frac{1}{2}", font_size=30, color=GREEN).move_to(axes_right.c2p(-0.2, 1/2))
        ...
        self.play(GrowFromPoint(y_line_left, axes_left.c2p(-np.sqrt(3)/2, 1/2)), GrowFromPoint(y_line, axes_right.c2p(np.sqrt(3)/2, 1/2)))
